...
class LocalStyleReader {
  final Logger logger;
  final String fileName;

  LocalStyleReader({required this.fileName, Logger? logger})
      : logger = logger ?? const Logger.noop();

  Future<Style> read() async {
    final File file = File(fileName);
    final styleText = await file.readAsString();
    final style = await compute(jsonDecode, styleText);
    if (style is! Map<String, dynamic>) {
      throw _invalidStyle(fileName);
    }
    ...
    */
    Map<String, VectorTileProvider> providerByName = <String, VectorTileProvider>{};
...
String _invalidStyle(String url) =>
    'File does not appear to be a valid style: $url';
